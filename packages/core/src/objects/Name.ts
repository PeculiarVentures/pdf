...
export class PDFName extends PDFString {
  ...
  protected onWritePDF(writer: ViewWriter): void {
    const escapedValue = this.text.replace(
      ESCAPE_REGEX,
      (
        substring // Encode irregular chars to '#dd'
  ...
  protected onFromPDF(reader: ViewReader): void {
    ...
    this.text = Convert.ToUtf8String(textView).replace(
      /#([0-9A-F]{2})/gim,
      (
        substring,
        group1: string // Encode '#dd' to char
