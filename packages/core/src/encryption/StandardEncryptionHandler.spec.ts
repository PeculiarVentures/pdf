...
} from "./StandardEncryptionHandler";
...
describe("StandardEncryptionHandler", () => {
  it("test checkUserPassword", async () => {
    pkijs.setEngine(
      "PDF",
      new PDFCryptoEngine({ name: "PDF", crypto: new Crypto() })
    );
    ...
    doc.update.xref!.set(
      ...
      doc.createArray(
        doc.createHexString(
          Convert.FromHex("EC0E03CF3746664B64CEF434F0E42A42")
        ),
    ...
    encrypt.set(
      ...
      doc.createHexString(
        Convert.FromHex(
          "173764629E9066BEB26F0DEBC7D0A71E40D386243BC28435CC9835F7BF552943"
        )
      )
    );
    encrypt.set(
      ...
      doc.createHexString(
        Convert.FromHex(
          "0D39108DA835B45179CC1B1C4B187EBA28BF4E5E4E758A4164004E56FFFA0108"
        )
      )
    );
    ...
    expect(doc.encryptHandler).toBeInstanceOf(StandardEncryptionHandler);
    const standardEncryptHandler =
      doc.encryptHandler as StandardEncryptionHandler;
    const ok = await standardEncryptHandler.checkUserPassword("12345");
    expect(ok).toBe(true);
  });
  ...
  it("test Adobe", async () => {
    ...
    doc.update.xref!.set(
      ...
      doc.createArray(
        doc.createHexString(
          Convert.FromHex("8D966A6EA9AE43E68A5B5E262C0AFEB5")
        ),
    ...
    encrypt.set(
      ...
      doc.createHexString(
        Convert.FromHex(
          "0eba1908e5cd53b188213637794ea65838027c93e38494b55544f4375b294c90"
        )
      )
    );
    encrypt.set(
      ...
      doc.createHexString(
        Convert.FromHex(
          "c827c9f262c92199c2208ef11b54294900000000000000000000000000000000"
        )
      )
    );
    encrypt.set("V", doc.createNumber(4));
    ...
    const standardEncryptHandler =
      doc.encryptHandler as StandardEncryptionHandler;
    standardEncryptHandler.onUserPassword = async () => "";

    const ok = await standardEncryptHandler.checkUserPassword("");
    expect(ok).toBe(true);
  });
  ...
  describe("create and encrypt document", () => {
    ...
    for (const t of tests) {
      it(t.name, async () => {
        ...
        stm.content.setColor(0).drawRectangle(10, 10, 20, 20).fill();
        ...
        const doc2 = await PDFDocument.fromPDF(new ViewReader(pdf));
        if (doc2.encryptHandler) {
          expect(doc2.encryptHandler).toBeInstanceOf(StandardEncryptionHandler);
          const standardEncryptHandler =
            doc2.encryptHandler as StandardEncryptionHandler;

          // check the User password
          const ok = await standardEncryptHandler.checkUserPassword(
            t.params.userPassword
          );
          expect(ok).toBe(true);

          standardEncryptHandler.onUserPassword = async (reason) => {
