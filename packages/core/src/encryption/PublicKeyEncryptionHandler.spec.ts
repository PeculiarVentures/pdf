...
describe("PublicKeyEncryptionHandler", () => {
  ...
  for (const t of tests) {
    it(t.name, async () => {
      ...
      if (t.params.file) {
        fs.writeFileSync(`${__dirname}/../../../../${t.params.file}.pdf`, pdf, {
          flag: "w+"
      ...
      if (doc2.encryptHandler) {
        expect(doc2.encryptHandler).toBeInstanceOf(PublicKeyEncryptionHandler);
        const publicKeyEncHandler =
          doc2.encryptHandler as PublicKeyEncryptionHandler;

        const keyParams = t.params.recipient.key;
        publicKeyEncHandler.onCertificate = async (_o) => {
          return {
