...
function wrapWidget(
  ...
  const FormComponentClass = FormComponentRegistry.get(
    type as FormComponentType.form
  );
...
function wrapField(
  ...
  const FormComponentGroupClass = FormComponentRegistry.get(
    type as FormComponentType.formGroup
  );
...
export class FormComponentFactory {
  ...
  protected static createFromWidget(
    widget: core.WidgetDictionary,
    doc: PDFDocument
  ): IComponent {
      ...
      if (filed.ff & core.ButtonFlags.radio) {
        ...
        if (widget.A.has()) {
          ...
          if (
            a.s === "JavaScript" &&
            a.has("JS") &&
            a.get("JS", core.PDFTextString).text.includes("buttonImportIcon")
          ) {
  ...
  protected static createFromField(
    field: core.PDFField,
    doc: PDFDocument
  ): IComponent {
      ...
      if (field.ft === "Btn") {
        if (field.ff & core.ButtonFlags.radio) {
          return wrapField(
            ...
            field.to(core.ButtonDictionary),
            doc
          );
        }
  ...
  public static create(dict: core.PDFDictionary, doc: PDFDocument): IComponent {
    ...
    throw new TypeError(
      "Cannot create PDF Form Component. Wrong type of PDF Dictionary"
    );
  }
}
